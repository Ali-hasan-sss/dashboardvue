<template>
    <v-container v-if="!isLoad" fluid class="icons-page">
                            ...
                            <span>
                                <v-avatar class="blue lighten-2" size="30">
                                    <span class="white--text">{{
                                        parseFloat(
                                            Number(item.office.rating).toFixed(
                                                1
                                            )
                                        )
                                    }}</span>
                        ...
                        <v-card :loading="false" class="mx-auto my-12">
                            ...
                            <v-card-text>
                                ...
                                >
                                    ...
                                    <span>
                                        {{
                                            estate.location.locations[0].name
                                        }}</span
                                    >
                                </div>
                                <div
                                ...
                                >
                                    ...
                                    <span>
                                        {{
                                            estate.estate_offer_type.name_ar
                                        }}</span
                                    >
                                </div>
                                <div
                                ...
                                >
                                    ...
                                    <span>
                                        {{
                                            estate.estate_type.name_ar.split(
                                                "|"
                                            )[1]
                                        }}</span
                                    >
                            ...
                            >
                                ...
                                <v-chip
                                    v-else-if="
                                        estate.system_estate_status.id == 2
                                    "
                                    class="ma-2 float-left white--text"
                                    color="red"
                                    outlined
        ...
        <v-dialog v-model="addEstateForm" max-width="500px">
            <addEstate
                v-if="addEstateForm"
                newItemLabel="المكتب"
                :isNew="isNew"
...
export default {
    ...
    components: { GeneralForm, exportToExcel, addEstate },
    methods: {
        formatDate(val) {
        ...
        openAddEstateDialog() {
            // فتح حوار إضافة عقار جديد
            this.addEstateForm = true;
            this.isNew = true;

            // إعداد النموذج للقيم الافتراضية
            this.$store.dispatch("setForm", {
                office_id: this.item.office.id,
                estate_id: null, // تأكد من تعيين null أو القيمة المناسبة
                name: "",
                price: null,
                location_id: null,
                estate_type_id: null,
                estate_offer_type_id: null,
                description: ""
                // أي بيانات إضافية ضرورية
            });
        },
        applyFilter() {
            ...
            switch (this.filterOption) {
                case "accepted":
                    this.filteredEstates = this.item.estates.data.filter(
                        estate => estate.system_estate_status.id == 1
                    );
                    break;
                case "rejected":
                    this.filteredEstates = this.item.estates.data.filter(
                        estate => estate.system_estate_status.id == 2
                    );
                    break;
                case "deleted":
                    this.filteredEstates = this.item.estates.data.filter(
                        estate => estate.system_estate_status.id == 3
                    ); // Assuming 'deleted' status id is 3
                    break;
                case "suspended":
                    this.filteredEstates = this.item.estates.data.filter(
                        estate => estate.system_estate_status.id == 4
                    ); // Assuming 'suspended' status id is 4
                    break;
                case "user_based":
                    this.filteredEstates = this.item.estates.data.filter(
                        estate => estate.user_based
                    ); // Assuming you have a 'user_based' property
                    break;
                case "all":
                default:
                    this.filteredEstates = this.item.estates.data;
                    break;
            }
        },
        populateTableData() {
